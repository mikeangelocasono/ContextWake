...
- Ubuntu WSL, Rust 1.88.0: format, strict Clippy, 70 tests, locked release build, Cargo package verification, and RustSec audit passed at the committed baseline.
- Windows, Rust 1.98.1/MSVC after Smart App Control was disabled: format and strict Clippy passed; 65 native baseline tests passed with no failures or ignored tests, including the formerly blocked Git integration executable.
- The baseline 6,388,736-byte Windows release executable ran version, status, and doctor from a separate directory. The real TUI rendered, navigated Help and Profiles, restored the terminal, and exited normally without an execution-blocking dialog.
