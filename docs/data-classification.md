...
| Provider credential reference/file/keychain item | SENSITIVE | Agent-owned; ContextWake does not read or persist its value |
...
The operating-system user account is the local trust boundary. File/directory permissions are restricted where supported, but ContextWake cannot protect data after the host account is compromised.
